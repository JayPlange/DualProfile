...
const DualProfileConfig = {
  ...
  // ── Pricing (GBP) ────────────────────────────────────────────────────────
  //
  // ONE paid tier. Pro — £29, one-time, no subscription.
  //
  // IMPORTANT — LEMON SQUEEZY SETUP:
  // This reuses the EXISTING Lifetime variant UUID rather than creating a new
  // one. In the Lemon Squeezy dashboard, reprice that variant from £79 to £29
  // and rename it "Pro". Reusing it means:
  //   • the checkout URL below does not change
  ...
  //
  // The old monthly (£9.99) and annual (£59) variants should be ARCHIVED in
  // Lemon Squeezy, not deleted — deleting them can break the receipt history
  // of anyone who already bought one.
  PRO_PRICE_GBP: 29,
  PRO_PRICE_DISPLAY: '£29',

  LEMONSQUEEZY_VARIANT_ID: '4f5df750-a085-44a6-8cdd-690b92bd80b1', // Pro £29 one-time

  PRO_CHECKOUT_URL: 'https://wadualpic.lemonsqueezy.com/checkout/buy/4f5df750-a085-44a6-8cdd-690b92bd80b1',

  /**
   * Get the Pro checkout URL (GBP £29, one-time).
   * @returns {string|null}
   */
  getCheckoutUrl() {
